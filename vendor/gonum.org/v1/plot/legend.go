...
func (l *Legend) Draw(c draw.Canvas) {
	...
	if l.Position == "bottom" {

		pad := l.TextStyle.Rectangle(" ").Max.X
		width := vg.Length(0)
		for i, e := range l.entries {
			width += l.ThumbnailWidth
			width += l.TextStyle.Rectangle(e.text).Max.X
			width += pad
			if i > 0 {
				width += 4 * pad
			}
		...
		offsetX := (c.Max.X + l.plotPadding - width) / 2
		offsetY := l.plotPadding + (l.TextStyle.Rectangle(" ").Max.Y / 2)
		...
		for i, e := range l.entries {
			...
			icon.Rectangle.Min.X = offsetX
			icon.Rectangle.Min.Y = offsetY
			icon.Rectangle.Max.X = offsetX + l.ThumbnailWidth
			icon.Rectangle.Max.Y = offsetY + (enth * 0.75)

			for _, t := range e.thumbs {
				t.Thumbnail(icon)
			}

			offsetX += l.ThumbnailWidth + pad
			offsetX += l.TextStyle.Rectangle(e.text).Max.X

			c.FillText(sty, vg.Point{X: offsetX, Y: offsetY}, e.text)
		}

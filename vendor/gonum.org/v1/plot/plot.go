...
func (p *Plot) Draw(c draw.Canvas) {
	...
	p.Padding = 5
	p.Legend.plotPadding = p.Padding
	...
	//
	plottersYOffs := vg.Length(0)
	if p.Legend.Position == "bottom" {
		plottersYOffs += p.Legend.TextStyle.Rectangle(" ").Max.Y * 2.0
		xheight += plottersYOffs
	} else {
		p.Legend.Position = "in" // Inside the chart, default by the bottom right corner
	}

	x.draw(padX(p, draw.Crop(c, ywidth, 0, plottersYOffs, 0)))
